...
def clean_content(content):
    ...
    content = re.sub(r'<[a-zA-Z/][^>]*>', ' ', content)           # strip tags before entity decode to preserve escaped HTML as text
    content = re.sub(r'\[/?[a-z_][^\]]*\]', ' ', content)         # remove WP shortcodes (lowercase names only — avoids [Note], [Update], etc.)
...
def process_xml_file(xml_file, writer):
    ...
    for item in channel.findall('item'):
        try:
            ...
            print(f"Warning: skipping item '{item.findtext('title', '')}' in {xml_file}: {e}", file=sys.stderr)
            skipped += 1
...
def main():
    ...
    xml_files = sorted(f for f in os.listdir(args.input_dir) if f.lower().endswith('.xml'))
    if not xml_files:
    ...
    try:
        with os.fdopen(tmp_fd, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for filename in xml_files:
                try:
                    skipped = process_xml_file(os.path.join(args.input_dir, filename), writer)
    ...
    if total_errors:
        print(f"Processing complete with {total_errors} error(s). Output saved to {args.output_file}", file=sys.stderr)
        sys.exit(1)
